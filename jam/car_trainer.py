...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', dest='n_epoch', type=int, default=25)
    ...
    train_val = np.load('data/jam_car_threat.npy')
    print('data size :', len(train_val))

    action = train_val[:, 5:7]
    idx = np.apply_along_axis(encode_action, 1, action).reshape(-1, 1)
    teacher = np.concatenate((idx, train_val[:, -1:]), axis=1)

    if args.gpu:
        train_val = xp.asarray(train_val)
        teacher = xp.asarray(teacher)
    ...
    chainer.serializers.save_npz('jam/car-threat.model', net)
